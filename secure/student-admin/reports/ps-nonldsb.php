...
<?php
	...
	if (isset($_GET['s']))
	{
		...
		$stmt = $conn->prepare("SELECT * FROM ss_registrations
								WHERE accepted_session = :accepted_session AND admin_non_ldsb = 1
	...
	if (isset($_GET['p']))
	{
		...
		$stmt = $conn->prepare("SELECT * FROM ss_registrations
								WHERE selected_program = :selected_program AND accepted_session != 0 AND admin_non_ldsb = 1
								ORDER BY student_name_last, student_name_first");
...
<body>
  ...
  <table align="center" style="font-size: .75em;">
  	<tr>
